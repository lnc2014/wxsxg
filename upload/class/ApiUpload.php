 ...
 *  支持单文件和多文件上传
 */
define('PATH', dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR.'statics/uploads/mobile/');
define('IMGPATH', 'statics/uploads/mobile/');
class ApiUpload{
		...
		if (is_array($name)){
			...
			if ($this->setFiles($name, $tmp_name, $size, $error)) {
				if ($this->checkFileSize() && $this->checkFileType()) {
					$this->setNewFileName();
					if ($this->copyFile()) {
						$Img_path = IMGPATH.date('Ymd').'/'.$this->newFileName;
						$array = array(
							'code'=>true,
							'path'=>$Img_path
						);
	...
	private function copyFile(){
		if (!$this->errorNum) {
			...
			$new_path.= $this->newFileName; 
			$this->new_path = $new_path;
			if (@move_uploaded_file($this->tmpFileName, $new_path)) {
				//返回成功，将路径也返回 
				return true; 
			}else{
